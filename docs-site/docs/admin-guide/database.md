...
Stores individual workloads within an estimate. This is a wide table with columns for all 14 workload types — only the relevant columns are populated for each type.
...
Defines the 14 supported workload types and their UI configuration.
