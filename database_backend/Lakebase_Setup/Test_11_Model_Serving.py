...
for key, combo in unique_combos.items():
    ...
    execute_query("INSERT INTO lakemeter.estimates (estimate_id, owner_user_id, estimate_name, description, cloud, region, tier, created_at, updated_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s);",
                  (estimate_id, TEST_USER_ID, f"Test: {combo['cloud']}", "Model Serving", combo['cloud'], combo['region'], combo['tier'], datetime.now(), datetime.now()), fetch=False)
...
results_df = execute_query("SELECT c.workload_name, c.cloud, c.tier, c.serverless_size, c.dbu_per_hour, c.dbu_price, c.dbu_cost_per_month, c.cost_per_month FROM lakemeter.v_line_items_with_costs c WHERE c.line_item_id = ANY(%s::uuid[]) ORDER BY c.display_order;", (line_item_ids,))

for col in ['dbu_per_hour', 'dbu_price', 'dbu_cost_per_month', 'cost_per_month']:
