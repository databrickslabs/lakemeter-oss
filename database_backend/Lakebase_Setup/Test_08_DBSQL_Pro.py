...
for key, combo in unique_combos.items():
    ...
    execute_query("INSERT INTO lakemeter.estimates (estimate_id, owner_user_id, estimate_name, description, cloud, region, tier, created_at, updated_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s);",
                  (estimate_id, TEST_USER_ID, f"Test: {combo['cloud']}", "DBSQL Pro", combo['cloud'], combo['region'], combo['tier'], datetime.now(), datetime.now()), fetch=False)
...
results_df = execute_query("SELECT c.workload_name, c.cloud, c.tier, c.dbsql_warehouse_type, c.dbsql_warehouse_size, c.dbsql_num_clusters, c.dbu_per_hour, c.dbu_price, c.cost_per_month FROM lakemeter.v_line_items_with_costs c WHERE c.line_item_id = ANY(%s::uuid[]) ORDER BY c.display_order;", (line_item_ids,))

for col in ['dbsql_num_clusters', 'dbu_per_hour', 'dbu_price', 'cost_per_month']:
