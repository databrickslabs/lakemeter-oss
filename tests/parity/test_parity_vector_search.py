...
class TestVectorSearchStorageOptimized:
    """Storage-optimized mode AI Search."""
...
class TestVectorSearchStorage:
    """AI Search storage sub-row parity."""

    def test_storage_within_free_tier(self, pricing):
        """5M standard → first 30 GB free → 30 GB storage → $0 billable."""
        item = make_item(
            workload_type='VECTOR_SEARCH', vector_search_mode='standard',
            vector_capacity_millions=5, vector_search_storage_gb=30,
            hours_per_month=730,
        )
        be_storage = _get_be_storage(item)
        fe_cost = fe_vector_search_storage_cost(storage_gb=30, units_used=3)
    ...
    def test_storage_exceeds_free_tier(self, pricing):
        """5M standard → first 30 GB free → 100 GB → 70 GB billable."""
        item = make_item(
            workload_type='VECTOR_SEARCH', vector_search_mode='standard',
    ...
    def test_storage_optimized_free_tier(self, pricing):
        """100M storage-optimized → first 30 GB free → 30 GB → $0."""
        item = make_item(
            workload_type='VECTOR_SEARCH', vector_search_mode='storage_optimized',
