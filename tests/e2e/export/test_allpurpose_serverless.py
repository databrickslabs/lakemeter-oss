"""E2E tests: All-Purpose Serverless — API calculation vs Excel export verification.
...
Run: pytest tests/e2e/export/test_allpurpose_serverless.py -v --timeout=600
"""
import pytest
...
def results():
    data = []
    yield data
    save_test_results(data, "test_results/e2e_allpurpose_serverless.json")


class TestAllPurposeServerlessCalculation:
    """Verify All-Purpose Serverless API calculation returns valid results."""
    ...
    def test_calculation_succeeds(self, e2e_client, cfg, mode, usage_label, usage, results):
        result = e2e_client.calculate_allpurpose_serverless(
...
class TestAllPurposeServerlessExcelExport:
    """Create estimates with All-Purpose Serverless workloads, export, verify."""
    ...
    def test_export_all_modes(self, e2e_client, cfg):
        ...
        estimate = e2e_client.create_estimate(
            name=f"E2E-AP-Serverless-{config_id(cfg)}",
        ...
        for mode in SERVERLESS_MODES:
            for usage_label, usage in [("hours_per_day", USAGE_HOURS_PER_DAY),
                                       ("hours_direct", USAGE_HOURS_DIRECT)]:
                api_result = e2e_client.calculate_allpurpose_serverless(
                    cloud=cloud, region=region, tier=tier,
                    mode=mode, usage=usage,
                )
                ...
                line_item_payload = {
                    "workload_name": f"AP-SL-{mode}-{usage_label}",
                    "workload_type": "ALL_PURPOSE",
                    "serverless_enabled": True,
                    "serverless_mode": mode,
                }
                if usage_label == "hours_per_day":
                    line_item_payload["hours_per_day"] = usage["hours_per_day"]
