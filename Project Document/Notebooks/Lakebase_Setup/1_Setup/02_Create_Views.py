...
required_tables = [
    ...
    ('lakemeter.sync_ref_dbsql_warehouse_config', 'DBSQL warehouse configuration table'),
...
-- Get DBSQL warehouse configuration (driver/worker instance types)
dbsql_config AS (
    SELECT 
        h.*,
        -- For DBSQL workloads, get driver/worker config from warehouse config table
        CASE WHEN h.workload_type = 'DBSQL' THEN dwc.driver_instance_type 
             ELSE h.driver_node_type END as resolved_driver_node_type,
        CASE WHEN h.workload_type = 'DBSQL' THEN dwc.worker_instance_type 
             ELSE h.worker_node_type END as resolved_worker_node_type,
        CASE WHEN h.workload_type = 'DBSQL' THEN dwc.driver_count 
             ELSE NULL END as dbsql_driver_count,
        CASE WHEN h.workload_type = 'DBSQL' THEN dwc.worker_count 
             ELSE h.num_workers END as resolved_num_workers
    FROM hours_calc h
    LEFT JOIN lakemeter.sync_ref_dbsql_warehouse_config dwc
        ON h.workload_type = 'DBSQL'
        AND dwc.cloud = h.cloud
        AND LOWER(dwc.warehouse_type) = LOWER(h.dbsql_warehouse_type)
        AND dwc.warehouse_size = h.dbsql_warehouse_size
),
...
classic_compute AS (
    SELECT 
        ...
        -- For DBSQL, use resolved driver/worker types from warehouse config
        ...
        CASE 
            WHEN dc.serverless_enabled THEN 1.0  -- Serverless: no multiplier (Photon included)
            ELSE COALESCE(m.multiplier, 1.0)    -- Classic: apply multiplier
        END as photon_multiplier
    FROM dbsql_config dc
    LEFT JOIN lakemeter.sync_ref_instance_dbu_rates d 
        ON d.cloud = dc.cloud AND d.instance_type = dc.resolved_driver_node_type
    LEFT JOIN lakemeter.sync_ref_instance_dbu_rates w 
        ON w.cloud = dc.cloud AND w.instance_type = dc.resolved_worker_node_type
    LEFT JOIN lakemeter.sync_ref_dbu_multipliers m 
        ON dc.serverless_enabled = FALSE
        AND m.cloud = dc.cloud
        AND m.feature = CASE WHEN dc.photon_enabled THEN 'photon' ELSE 'standard' END
        AND m.sku_type = CASE 
            WHEN dc.workload_type = 'DLT' THEN 'DLT_' || COALESCE(dc.dlt_edition, 'CORE') || '_COMPUTE'
            WHEN dc.workload_type = 'JOBS' THEN 'JOBS_COMPUTE'
            WHEN dc.workload_type = 'ALL_PURPOSE' THEN 'ALL_PURPOSE_COMPUTE'
            ELSE 'JOBS_COMPUTE'
        END
),
...
final_calc AS (
    SELECT 
        ...
        -- Include DBSQL Classic/Pro (not Serverless)
        CASE WHEN (p.workload_type IN ('ALL_PURPOSE', 'JOBS', 'DLT') AND p.serverless_enabled = FALSE)
                   OR (p.workload_type = 'DBSQL' AND p.dbsql_warehouse_type IN ('CLASSIC', 'PRO')) THEN
            COALESCE((
                SELECT cost_per_hour FROM lakemeter.sync_pricing_vm_costs 
                WHERE cloud = p.cloud AND region = p.region 
                AND instance_type = p.resolved_driver_node_type
        ...
        CASE WHEN (p.workload_type IN ('ALL_PURPOSE', 'JOBS', 'DLT') AND p.serverless_enabled = FALSE)
            ...
            COALESCE((
                SELECT cost_per_hour FROM lakemeter.sync_pricing_vm_costs 
                WHERE cloud = p.cloud AND region = p.region 
                AND instance_type = p.resolved_worker_node_type
...
SELECT 
    ...
    -- Original node types (NULL for DBSQL)
    driver_node_type, worker_node_type, num_workers,
    -- Resolved node types (includes DBSQL warehouse config)
    resolved_driver_node_type, resolved_worker_node_type, resolved_num_workers,
    -- DBSQL-specific driver count
    dbsql_driver_count,
